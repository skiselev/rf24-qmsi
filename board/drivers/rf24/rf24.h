 ...
 * @file rf24.h
 *
 * Function declaration for rf24 and helper enums
 */
...
#endif
bool rf24_plus;						/* False for RF24L01 and true for RF24L01P */
uint8_t rf24_payload_size;			/* Fixed size of payloads */
bool rf24_dynamic_payloads_enabled;	/* Whether dynamic payloads are enabled. */
uint8_t rf24_addr_width;			/* The address width to use - 3,4 or 5 bytes. */
#if defined (RF24_FAILURE_HANDLING)
    bool rf24_failure_detected; 
#endif

uint8_t pipe0_reading_address[5];	/* Last address set on pipe 0 for reading. */
uint32_t rf24_tx_rx_delay;			/* Used for adjusting delays depending on datarate */

uint8_t rf24_spi_transfer(uint8_t in);

#if defined(RF24_SPI_MULTIBYTE)
uint8_t rf24_spi_transfernb(uint8_t *tx_buf, uint8_t *rf_buf, uint8_t len);
#endif

/**
 * @name Public interface
 *
 *  These are the main functions you need to operate the chip
 */
/**@{*/
 ...
 * 2. Do not call rf24_write() while in this mode, without first calling rf24_stop_listening().
 * 3. Call rf24_available() to check for incoming traffic, and rf24_read() to get it. 
 *  
 * @code
 * Open reading pipe 1 using address CCCECCCECC
 *  
 * byte address[] = { 0xCC,0xCE,0xCC,0xCE,0xCC };
 * rf24_open_reading_pipe(1, address);
 ...
 * }
 * @endcode
 * @return No return value. Use rf24_available().
 */
 ...
 *   rf24_open_writing_pipe(addresses[0]);
 * @endcode
 * @code
 *  uint8_t address[] = {0xCC, 0xCE, 0xCC, 0xCE, 0xCC};
 *  rf24_open_writing_pipe(address);
 *  address[0] = 0x33;
 *  rf24_open_reading_pipe(1, address);
 * @endcode
 * @see setAddressWidth
...
void rf24_open_reading_pipe(uint8_t number, const uint8_t *address);

/**@}*/
  /**
   * @name Advanced Operation
   *
   *  Functions you can use to drive the chip in more advanced ways
   */
/**@{*/

/**
 * FIXME: Fix the description here
 ...
 *
 * @note After calling rf24_start_listening(), a basic radio will consume about 13.5mA
 * at max PA level.
 * During active transmission, the radio will consume about 11.5mA, but this will
 * be reduced to 26uA (.026mA) between sending.
 * In full rf24_power_down mode, the radio will consume approximately 900nA (.0009mA)   
 *
 * @code
 * rf24_power_down();
 * soc_sleep();
 ...
 *
 * @note rf24_enable_dynamic_ack() must be called to enable this feature
 *
 * Can be used with rf24_enable_ack_payload() to request a response
 * @see rf24_enable_dynamic_ack()
 * @see rf24_set_auto_ack()
 * @see rf24_write()
 *
 * @param buf Pointer to the data to be sent
 * @param len Number of bytes to be sent
 ...
 * Once the FIFOs are full, rf24_write_fast will simply wait for success or
 ...
 * to clear by issuing rf24_tx_standby() or ensure appropriate time between transmissions.
 *
 * @code
 * Example (Partial blocking):
 *
 *			rf24_write_fast(&buf, 32); // Writes 1 payload to the buffers
 *			rf24_tx_standby();    	   // Returns 0 if failed. 1 if success. Blocks only until MAX_RT timeout or success. Data flushed on fail.
 *
 *			rf24_write_fast(&buf, 32); // Writes 1 payload to the buffers
 *			rf24_tx_standby(1000);	   // Using extended timeouts, returns 1 if success. Retries failed payloads for 1 seconds before returning 0.
 * @endcode
 *
 * @see rf24_tx_standby()
 * @see rf24_write()
 * @see rf24_write_blocking()
 *
 * @param buf Pointer to the data to be sent
 * @param len Number of bytes to be sent
 * @return True if the payload was delivered successfully false if not
 ...
 *
 *			rf24_write_blocking(&buf, 32, 1000); //Wait up to 1 second to write 1 payload to the buffers
 *			rf24_tx_standby(1000);     			 //Wait up to 1 second for the payload to send. Return 1 if ok, 0 if failed.
 ...
 * Example (Partial blocking):
 *
 *			rf24_write_fast(&buf, 32);
 *			rf24_write_fast(&buf, 32);
 *			rf24_write_fast(&buf, 32);   //Fills the FIFO buffers up
 *			bool ok = rf24_tx_standby(); //Returns 0 if failed. 1 if success.
 *					  				     //Blocks only until MAX_RT timeout or success. Data flushed on fail.
 * @endcode
 * @see rf24_tx_standby_timeout
 ...
 *			rf24_write_fast(&buf, 32);
 *			rf24_write_fast(&buf, 32);           //Fills the FIFO buffers up
 *			bool ok = rf24_tx_standby(1000, 0);  //Returns 0 if failed after 1 second of retries. 1 if success.
 ...
 * @see rf24_enable_ack_payload()
 * @see rf24_enable_dynamic_payloads()
 ...
 * write an ack payload as soon as rf24_start_listening() is called, so one is available when a regular payload is received.
 * @note Ack payloads are dynamic payloads. This only works on pipes 0&1 by default. Call 
 * rf24_enable_dynamic_payloads() to enable on all pipes.
 *
 ...
 * @return True if the payload was delivered successfully false if not
 */
void rf24_start_fast_write(const void* buf, uint8_t len, const bool multicast, bool startTx);
 ...
 * After issuing rf24_reuse_tx(), it will keep reending the same payload forever or until
 * a payload is written to the FIFO, or a rf24_flush_tx command is given.
 */
void rf24_reuse_tx();
 ...
 *
 * @code
 * bool good_signal = rf24_test_rpd();
 * if(rf24_available()){
 *    printf(good_signal ? "Strong signal > 64dBm" : "Weak signal < 64dBm");
 *    rf24_read(0, 0);
 * }
 * @endcode
...
/**
 * Enable error detection by un-commenting #define RF24_FAILURE_HANDLING in rf24_config.h
 ...
 *  @code
 *  if(rf24_failure_detected){ 
 *    rf24_init();                          // Attempt to re-configure the radio with defaults
 *    rf24_failure_detected = 0;            // Reset the detection value
 *	  rf24_open_writing_pipe(addresses[1]); // Re-configure pipe addresses
 *    rf24_open_reading_pipe(1, addresses[0]);
 ...
 *
 *  Functions you can use to get or set the configuration of the chip.
 ...
 * @code
 * if(rf24_available()){
 *   if(rf24_get_dynamic_payload_size() < 1){
 ...
 * @code
 * rf24_enable_dynamic_ack();
 * rf24_write_FIXME(&data, 32, 1);  // Sends a payload with no acknowledgement requested
 * rf24_write_FIXME(&data, 32, 0);  // Sends a payload using auto-retry/autoACK
 * @endcode
 */
void rf24_enable_dynamic_ack();
 ...
 */
void rf24_mask_irq(bool tx_ok,bool tx_fail,bool rx_ready);
 ...
 * @code
 *   rf24_open_reading_pipe_uint64(1, 0xF0F0F0F0AA);
 *   rf24_open_reading_pipe_uint64(2,0xF0F0F0F066);
 * @endcode
 *
 * @warning Pipe 0 is also used by the writing pipe.  So if you open
 ...
 */
void rf24_open_reading_pipe_uint64(uint8_t number, uint64_t address);
 ...
 *
 * @code
 *   rf24_open_writing_pipe_uint64(0xF0F0F0F0F0);
 * @endcode
 *
 * @param address The 40-bit address of the pipe to open.
 */
void rf24_open_writing_pipe_uint64(uint64_t address);
 ...
 * @return Current value of status register
 */
uint8_t rf24_write_payload(const void* buf, uint8_t len, const uint8_t write_type);
...
uint8_t rf24_spi_trans(uint8_t cmd);
  
#if defined (RF24_FAILURE_HANDLING)
void rf24_err_notify(void);
#endif
